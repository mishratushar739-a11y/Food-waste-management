...
def get_connection_params():
    ...
    Also returns "_source" so the UI can tell the user where each
    value came from (secrets vs. env vars vs. hardcoded fallback) —
    this is what makes a silent "localhost" fallback visible instead
    of just failing mysteriously.
    """
    secrets_mysql = {}
    try:
    ...
    using_secrets = bool(secrets_mysql)
    using_env = (not using_secrets) and any(
    ...
    return {
        ...
        "_source": "secrets" if using_secrets else ("env" if using_env else "fallback"),
    }
...
def sidebar_connection():
    ...
    source = defaults.pop("_source")
    source_label = {
        "secrets": "✅ Loaded from Streamlit Secrets",
        "env": "✅ Loaded from environment variables",
        "fallback": "⚠️ No secrets/env vars found — using hardcoded local defaults (won't work when deployed)",
    }[source]
    st.sidebar.caption(source_label)

    with st.sidebar.expander("MySQL credentials", expanded=(source == "fallback")):
        ...
        st.caption(
            "Tip: set these once in Streamlit Cloud → Settings → Secrets under "
            "`[mysql]` so you don't have to re-enter them each run."
        )
    ...
    st.session_state["_db_last_error"] = last_error
    st.session_state["_db_source"] = source
...
def main():
    ...
    if not connected:
        ...
        source = st.session_state.get("_db_source")
        last_error = st.session_state.get("_db_last_error")

        if source == "fallback":
            st.warning(
                "**No `[mysql]` secrets were found.** This app is using hardcoded "
                "local defaults (`localhost`), which will never work on Streamlit "
                "Community Cloud since there's no MySQL server on the cloud "
                "container itself. Go to **Manage app → Settings → Secrets** and "
                "add your database credentials there."
            )

        if last_error is not None:
            with st.expander("Show full connection error", expanded=True):
                st.code(str(last_error), language="text")
                msg = str(last_error)
                if "railway.internal" in msg:
                    st.info(
                        "This looks like a Railway **internal** hostname "
                        "(`*.railway.internal`). That address only works for "
                        "services inside Railway's own network — it can't be "
                        "reached from Streamlit Cloud. In Railway, enable "
                        "**Public Networking / TCP Proxy** on your MySQL service "
                        "and use that public host + port instead."
                    )
                elif "Access denied" in msg:
                    st.info("Your username or password is incorrect for this database.")
                elif "Unknown database" in msg:
                    st.info("The database name doesn't exist on this server yet.")
                elif "Can't connect" in msg or "timed out" in msg.lower():
                    st.info(
                        "The host/port couldn't be reached at all — double check "
                        "they're correct, and that the database's public networking "
                        "is enabled and not firewalled."
                    )
